...
//This extra feature program has a special display for when you win with all turns intact, has sound, and waits for the user to click.  
...
public class BreakoutExtra extends BreakoutProgram {
	...
	//In this method, the user clicks to serve the ball and begin the game 
	//There is sound integrated for added effect

	public void clickToServeBall(){
		GLabel clickToPlay  = new GLabel("CLICK TO PLAY!", 0,0);
		add(clickToPlay);
		clickToPlay.setFont(SCREEN_FONT);
		clickToPlay.setLocation(getWidth()/2 - clickToPlay.getWidth()/2, getHeight()/2 - clickToPlay.getHeight()/2);
		waitForClick();
		remove(clickToPlay);
	}

	//This method is the bounce sound that plays whenever you serve 
	...
	public void collisions(GOval ball, GLabel display){
		...
		if(collider == paddle){
			vy = -Math.abs(vy);   //takes care of sticky paddle
			bounceSound();
	...
	public void bouncingBall(GOval ball, GLabel display, int turns){	
		...
		while(true){
			...
			if(bricksLeft == 0){ // Calling the win method 
				...
				if(bricksLeft == 0 && turns == 3){               //runs only if you're good 
					GLabel champion = new GLabel("You are indeed a Champion.", 0,0);
					add(champion);
					champion.setFont(SCREEN_FONT);
					champion.setLocation(getWidth()/2 - champion.getWidth()/2, getHeight()/2 + 1.2*champion.getHeight());
					break;
				}
				break;
			}
	...
	//This method creates the paddle (works for Macs but not Windows for some reason)
